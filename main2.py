...
def render_tasks():
    ...
    for task in ordered:
        group_name = task.get("group", "").strip()

        if not group_name:
            group_name = "Sans projet"

        groups.setdefault(group_name, []).append(task)
    ...
    for group_name, group_tasks in groups.items():
        ...
        dpg.add_spacer(height=10, parent="task_list_group")
            ...
            with dpg.child_window(
                ...
                with dpg.group(horizontal=True):
                    ...
                    if task["done"]:
                        ...
                        dpg.bind_item_theme(
                            ico,
                            make_text_theme(TEXT_MID)
                        )

                    dpg.add_spacer(width=4)

                    # Texte tâche
                    # Texte + sélection du projet
                    with dpg.group(horizontal=True):
                        ...
                        all_projects = sorted(
                        {
                            t.get("group", "Sans projet")
                            for t in tasks
                        }
                    )

                    if not all_projects:
                        all_projects = ["Sans projet"]

                    dpg.add_spacer(width=15)

                    dpg.add_combo(
                        items=all_projects,
                        default_value=task.get("group", "Sans projet"),
                        width=120,
...
def add_task(sender=None, app_data=None, user_data=None):
    ...
    tasks.append({
    "text": raw,
    "group": dpg.get_value("group_input"),
    "done": False,
    "priority": False,
...
def move_task(idx, new_group):
    if 0 <= idx < len(tasks):
        tasks[idx]["group"] = new_group
        render_tasks()
